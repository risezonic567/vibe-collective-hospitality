    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between">
        <Link to="/" className="group flex flex-col">
          <span className="font-serif text-xl md:text-2xl tracking-[0.2em] text-[#FAF9F6] font-light uppercase group-hover:text-[#D4AF37] transition-colors">
            Vibe Collective
          </span>
            ...
            return (
              <Link
                ...
                className={`text-xs uppercase tracking-[0.2em] transition-colors duration-300 relative py-1 ${
                  isActive ? "text-[#D4AF37]" : "text-[#FAF9F6]/80 hover:text-[#FAF9F6]"
                }`}
              >
                {link.name}
                {isActive && (
